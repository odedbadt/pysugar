...
def _hist(np_hist, title="Histogram", x=None, pdf=None, cdf=None, quarentiles=False):
    ...
    p = figure(title=title, width=600, tools='pan,wheel_zoom,box_zoom,save', background_fill_color="#fafafa")
...
def hist(data, bins='auto', title="Histogram", x=None):
    v = np.array(data)
    v = v[~(np.isnan(v) | np.isinf(v))]
    _hist(np.histogram(v, bins=bins), title, x)
